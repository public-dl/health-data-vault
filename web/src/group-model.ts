...
export function groupTableText(data:Payload,group:IndicatorGroup,code:string) {
  ...
  return [`地域\t実績年度\t${group.categories.map(c=>c.label).join('\t')}\t受診者数（人）\t公表年度\t検証・比較`,...data.years.map(year=>{
    ...
    return [name,year,...group.categories.map((_,i)=>part?`${formatValue(part.rows[i].value,part.rows[i].unit)}${part.rows[i].unit}（${numerator(part.rows[i])}人）`:'表示不可'),part?.denominator??'',part?.rows[0].publication_fiscal_year??'',part?`passed / ${part.rows[0].comparability_status}`:'保留'].join('\t');}),
...
export function transposedGroupTableText(data:Payload,group:IndicatorGroup,code:string,years:number[]) {
 ...
 return [name+' / '+group.name,'区分\t'+years.map(y=>y+'年度').join('\t'),
  ...
  ...group.categories.map((cat,i)=>[cat.label,...parts.map(p=>p?`${formatValue(p.rows[i].value,p.rows[i].unit)}${p.rows[i].unit}（報告人数 ${numerator(p.rows[i])}人・${p.rows[i].comparability_status==='pending'?'年度間比較は確認中':'検証済・比較可能'}）`:'表示不可')].join('\t')),
