...
const sources={
 ...
 program:{title:'厚生労働省「標準的な健診・保健指導プログラム（令和6年度版）」',url:'https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000194155_00004.html'},
};
export const learningSections:[string,string,(keyof typeof sources)[]][]=[
 ...
 ['医師の判断','新潟県の原表は「異常認めず」「保健指導」「受診勧奨」を別々の報告区分として掲載しています。本サイトでは医師の個別判断を検査値から再判定せず、報告された人数と単年度構成を表示します。異常認めずは、将来の疾病がないという保証ではありません。',['program','results']],
 ['メタボ判定','腹囲と血圧・脂質・血糖等のリスクの組合せに着目した判定です。新潟県原表の非該当・予備群・基準該当・判定不能を、そのまま区分として使用します。医師の総合判断や特定保健指導の階層化とは別の集計軸です。',['program','stratification']],
