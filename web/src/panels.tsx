...
export function Insight({c,code,map=false}:{c:Context;code:string;map?:boolean}) {
  const result=c.data.insights.find(i=>(i.measure??'count')===c.measure&&i.indicator_id===c.indicator.indicator_id&&i.geography_code===code&&i.observation_fiscal_year===(map?c.year:c.data.years.at(-1)));
  return <DataNotes group={!map}><p>{map?localMapText(result?.text ?? '説明は準備中です。'):(result?.text ?? '説明は準備中です。')}</p>{!map&&<small>{map?(c.measure==='rate'?'色は受診者に占める割合です。年齢・性別構成を調整していません。':'色は報告人数の大小です。受診者の規模に左右されます。'):'対象年度：'+c.data.years.join('・')+'年度。'+(isReportedSchema(c.data.schema_version)?'年度間比較はpendingです。':'割合は年齢・性別構成を調整していません。')}</small>}</DataNotes>;
}
...
export function GraphPanel({c}:{c:Context}) {
  ...
  return <ExportSurface kind="graph-card" className="panel"><div className="panel-heading"><div><h3>選択地域の推移</h3><IndicatorBadge c={c}/></div><Actions c={c} svg={ref} rows={rows} title={title}/></div>
    ...
    </svg></div><p className="chart-scroll-hint">グラフは横にスクロールして全年度を確認できます。</p><SourceLink c={c} rows={all.flatMap(s=>s.rows)}/></div><DataNotes>{series.map(s=>{const first=s.rows[0],last=s.rows.at(-1);const comparable=s.rows.length>1&&s.rows.slice(1).every((r,i)=>connect(s.rows[i],r));return <p key={s.code}>{nameFor(c,s.code)}：{last?.observation_fiscal_year}年度は{formatValue(last?.value,c.indicator.unit)}{c.indicator.unit}。{comparable&&first.value!=null&&last?.value!=null&&`${first.observation_fiscal_year}年度からの変化は${last.value-first.value>=0?'+':''}${formatValue(last.value-first.value,c.indicator.unit)}${c.measure==='rate'?'ポイント':'人'}です。`}</p>;})}{delta!=null&&<p>{years.at(-1)}年度の{ nameFor(c,series[1].code)} − {nameFor(c,series[0].code)}：{delta>=0?'+':''}{formatValue(delta,c.indicator.unit)}{c.measure==='rate'?'ポイント':'人'}。</p>}<small>差は丸め前の値から算出しています。健康状態の優劣や政策効果を示しません。{c.regions.some(code=>c.data.geographies.find(g=>g.code===code)?.level==='prefecture_total')&&'県計には各市町村が含まれます。'}</small></DataNotes></div><p className="footnote">{measureLabel(c)}の推移です。欠損・比較不可の年度間は接続しません。割合は年齢・性別構成を調整していません。</p></ExportSurface>;
}
