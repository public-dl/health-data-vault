import {regionMap} from './region-hierarchy';
...
export function Insight({c,code,map=false}:{c:Context;code:string;map?:boolean}) {
  ...
  const current=rowsFor(c,code).find(r=>r.observation_fiscal_year===c.year);
  const fallback=current?.geography_level==='health_center_area'?`${c.year}年度の${nameFor(c,code)}：${c.indicator.name}は${formatValue(current.value,current.unit)}${current.value==null?'':current.unit}です。年度間比較は確認中です。`:'説明は準備中です。';
  const text=refreshTemporalComment(result?.text ?? fallback,rowsFor(c,code),c.year);
...
export function MapPanel({c,region,onSelect,compact=false,highlightRegion,onHighlightRegion,showNotes=true}:{c:Context;region:string;onSelect:(s:string)=>void;compact?:boolean;showNotes?:boolean;highlightRegion?:string;onHighlightRegion?:(code:string)=>void}) {
  const level=c.data.geographies.find(g=>g.code===region)?.level;const code=level==='municipality'||level==='health_center_area'?region:undefined;const map=regionMap(c.data,region);
  ...
  const projection=useMemo(()=>geoMercator().fitExtent([[60,70],[620,445]],map),[map]);
  const path=useMemo(()=>geoPath(projection),[projection]);
  const title=code?nameFor(c,code)+'（県内での位置）':`新潟県 全${map.features.length}市町村の分布`;
  const rows=c.data.records.filter(r=>r.indicator_id===c.indicator.indicator_id&&r.observation_fiscal_year===c.year&&r.geography_level===(level==='health_center_area'?'health_center_area':'municipality'));
  ...
  return <ExportSurface kind="map-card" className="panel map-card"><div className="panel-heading map-card-heading"><div><span className="eyebrow">{level==='health_center_area'?'HEALTH CENTER AREA':code?'SELECTED MUNICIPALITY':'NIIGATA PREFECTURE'}</span><h3 className="map-region-title">{title}{!code&&`（${c.year}年）`}</h3><IndicatorBadge c={c}/></div><MapSummary record={selected} measure={c.measure} rateLabel={c.indicator.rate?.label} recipientLabel={c.indicator.recipient_label} color={(indicatorVisual(c.data.indicator_groups,c.indicator.indicator_id)?.visual??indicatorColorAliases[c.indicator.indicator_id])?.color}/><Actions c={c} svg={ref} rows={rows} title={`${title}_${c.year}年度`}/></div>
    ...
    {code&&<p className="footnote">選択地域を濃く強調しています。地域内の地区別データはありません。</p>}
  </ExportSurface>;
}
