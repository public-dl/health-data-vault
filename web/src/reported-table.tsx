...
export function ReportedSetTable({c,code,members}:{c:Context;code:string;members:string[]}) {
 ...
 <div className="group-table-body"><div className="table-scroll"><CategoryYearTable years={years} selectedYear={c.year} publications={years.map(y=>record(members[0],y)?.publication_fiscal_year)} caption={countOnly?setLabel+" / 報告人数（人）":setLabel+" / 特定健診受診者に占める割合（%）"+(indicators.some(i=>!i.rate)?"・割合未承認項目は人数のみ":"")}>
 ...
 </CategoryYearTable></div><p className="footnote">{countOnly?"原表の報告人数です。項目間の合計・残差・地域差は生成しません。年度間比較は確認中です。":"各割合は報告人数÷同年度・同地域の特定健診受診者数×100。各項目は完全な構成ではありません。年度間比較は確認中です。"}</p><SourceLink c={c} rows={rows}/>
 <svg ref={ref} className="export-only" aria-hidden="true" viewBox={`0 0 ${width} ${height}`}><rect width={width} height={height} fill="white"/><text x="20" y="30" {...svgText} fontSize="18">{nameFor(c,code)} / {setLabel}</text>{years.map((y,j)=><g key={y}><text x={270+j*180} y="64" {...svgText} textAnchor="middle" fontSize="18">{y}年度</text><text x={270+j*180} y="84" {...svgText} textAnchor="middle" fontSize="11">公表 {record(members[0],y)?.publication_fiscal_year}年度</text></g>)}{indicators.map((i,k)=><g key={i.indicator_id}><text x="20" y={120+k*115} {...svgText} fontSize="15">{i.name}</text>{years.map((y,j)=>{const r=record(i.indicator_id,y),rate=reportedRate(r);return <g key={y}><text x={270+j*180} y={120+k*115} {...svgText} textAnchor="middle" fontSize="18">{rate?formatValue(rate.value,'%')+'%':r?format(r.value)+'人':'未収録'}</text><text x={270+j*180} y={146+k*115} {...svgText} textAnchor="middle" fontSize="18">{rate?format(rate.numerator_value)+'人':''}</text><text x={270+j*180} y={168+k*115} {...svgText} textAnchor="middle" fontSize="12">参考：受診者 {format(rate?.denominator_value??r?.denominator?.value)}人</text><text x={270+j*180} y={187+k*115} {...svgText} textAnchor="middle" fontSize="11">年度間比較 確認中</text></g>;})}</g>)}</svg>
 <DataNotes group>{indicators.map(i=>{const rate=reportedRate(record(i.indicator_id,c.year));return <p key={i.indicator_id}>{c.year}年度の{nameFor(c,code)}：{i.name} {rate?`${formatValue(rate.value,'%')}%（${format(rate.numerator_value)}人／特定健診受診者 ${format(rate.denominator_value)}人）`:record(i.indicator_id,c.year)?`${format(record(i.indicator_id,c.year)?.value)}人。参考：特定健診受診者数 ${format(record(i.indicator_id,c.year)?.denominator?.value)}人`:'未収録'}。</p>;})}</DataNotes>
 </div></ExportSurface>;
}
