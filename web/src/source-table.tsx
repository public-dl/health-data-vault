...
export function tableLink(r:Observation,review:boolean){const p=new URLSearchParams({year:String(r.observation_fiscal_year),cell:r.source_cell});if(review)p.set('review','1');return '/tables?'+p;}
...
export function PublishedTables({tables,review,release}:{tables:PublishedTable[];review:boolean;release:string}) {
 ...
 const selectYear=(y:number)=>{setYear(y);const p=new URLSearchParams(location.search);p.set('year',String(y));history.replaceState(null,'','/tables?'+p);};
 const choose=(c:TableCell)=>{setCell(c.coordinate);const p=new URLSearchParams(location.search);p.set('year',String(year));p.set('cell',c.coordinate);history.replaceState(null,'','/tables?'+p);};
 return <main className="published-page"><a href={'/'+(review?'?review=1':'')}>← 地域比較へ</a><h1>公表数表</h1><p>特定健康診査等結果報告【市町村国保】（市町村別集計表）</p>{review&&<p className="review-banner">未承認・ローカル確認用</p>}<nav className="year-tabs" aria-label="公表数表の実績年度">{tables.map(t=><button key={t.source.observation_fiscal_year} aria-pressed={year===t.source.observation_fiscal_year} onClick={()=>selectYear(t.source.observation_fiscal_year)}>{t.source.observation_fiscal_year}年度</button>)}</nav><p>出典：<a href={table.source.annual_page_url}>健康にいがた21</a> ／ 掲載 {table.source.publication_fiscal_year}年度 ／ <a href={table.source.source_url}>原典Excelを開く</a></p><div className="actions"><button onClick={async()=>{try{await navigator.clipboard.writeText(sourceTableText(table));setMessage('原表をコピーしました');}catch{setMessage('コピーできません。CSV保存をご利用ください。');}}}>表をコピー</button><button onClick={()=>save(new Blob(['\uFEFF'+sourceTableText(table,',')],{type:'text/csv;charset=utf-8'}),`公表数表_${year}.csv`)}>CSV保存</button></div><p>表は縦・横にスクロールできます。原表の行順・再掲を保持しています。合計行と内訳を加算しないでください。ハイフン表示には数値0の表示形式も含まれます。セルを選ぶと実値・型・表示形式を確認できます。</p>
